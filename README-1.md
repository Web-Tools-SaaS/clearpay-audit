...
obligation flows down to merchants contractually.
...
[Audit Pipeline — runs async via after()]
  ...
  Step 5: runRuleEngine(text, provider) — all 17 rules evaluated
...
lib/rule-engine/
  ...
  rules.ts        — All 17 Rule objects
  evaluateRule.ts — Single rule evaluator (handles invert flag + UNCLEAR status)
...
#### CRITICAL (5 rules) — 20 points deducted each on FAIL
...
#### HIGH (7 rules) — 10 points deducted each on FAIL

| ID | Category |
|---|---|
| DPC-006 | Right to Withdraw — 14-day right, accessible in linked terms (moved from KPI in PS26/1 final rules) |
...
| DPC-015 | Rights Signpost — KPI must signpost to where rights are set out (new in PS26/1) |

#### MEDIUM (5 rules) — 5 points deducted each on FAIL

| ID | Category |
|---|---|
| DPC-012 | Continuous Payment Authority (CPA) — how auto-payments work |
| DPC-013 | Free Debt Advice Signposting — MoneyHelper, StepChange etc. (new in PS26/1) |
| DPC-014 | FCA Authorisation — provider must be FCA-authorised by July 15 |
| DPC-016 | Credit Reference Agency Check — disclosure of CRA check (amended in PS26/1) |
| DPC-017 | FOS Complaint Rights — access to Financial Ombudsman Service (new in PS26/1) |

> **Note on rule numbering:** DPC-015 through DPC-017 are numbered above DPC-014
> because they were added after the initial 14-rule build. IDs are permanent
> identifiers tied to FCA clause references — do not renumber.

### The Inverted Rule (DPC-010)

All 16 standard rules: search for required strings → PASS if found, FAIL if absent.
...
When crawled text is fewer than 200 words, a FAIL result is downgraded to
UNCLEAR. This reflects genuine uncertainty rather than a confirmed absence.
Inverted rules (DPC-010) are exempt — finding prohibited language on a thin
page is a valid FAIL regardless. UNCLEAR deducts half the severity weight.
...
UNCLEAR:       half of severity weight deducted
Minimum:        0
```
...
| bnpl_provider | text | 'klarna' \| 'clearpay' \| 'paypal' \| 'other' |
...
regulation deadline badge, how it works (3 steps), what we check (17 rules
...
| Page too thin for reliable FAIL | UNCLEAR status returned when < 200 words — half deduction, prompts resubmit |
